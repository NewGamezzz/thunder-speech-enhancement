    ...
    def reverse(oself, score_model, probability_flow=False):
        ...
        class RSDE(oself.__class__):
            ...
            def discretize(self, x, t, y, stepsize):
                ...
                # print("X", x)
                # print("Y", y)
                # print("REVERSE SDE", score_model(x, t, y))
...
class OUVESDE(SDE):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument(
            "--sigma-max", type=float, default=0.5, help="The maximum sigma to use. 0.5 by default."
        )
        return parser
...
class BB(SDE):
    ...
    def sde(self, x, t, y):
        assert t != 1, "Time step must not equal to 1. Please set to 0.999."
        diffusion = torch.ones_like(t)
        t = t[:, None, None, None]
        drift = (y - x) / (1 - t)
        return drift, diffusion
...
class BBED(SDE):
    ...
    def add_argparse_args(parser):
        ...
        parser.add_argument(
            "--theta", type=float, default=0.52, help="root scale factor for diffusion term."
        )
        return parser
